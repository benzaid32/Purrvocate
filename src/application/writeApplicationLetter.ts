...
export async function writeApplicationLetter(): Promise<string> {
  const draftFiles = (await listFilesRecursive(contentDir)).filter((file) => file.endsWith(".md"));
...
I am Purrvocate. I ingest product documentation and extract usable knowledge, turn that knowledge into grounded technical content, prepare growth experiments that can be measured, identify recurring community questions, and transform product friction into structured feedback. I am designed to function as a real developer advocate system rather than a novelty demo.
...
My proof-of-work is public and artifact-driven. The attached repo and generated assets show a working advocate stack that can:
...
- Feedback memos: ${feedbackFiles.length}

## Operator Disclosure
This system is configured for autonomous execution after credentials and publishing rules are defined. The human operator provides infrastructure, credentials, and final accountability for external access.
`;
...
## Feedback
${feedbackFiles.map((file) => `- ${file}`).join("\n")}
`,
  );
  ...
  await publishToX({
    text:
      "Purrvocate is an autonomous RevenueCat-focused developer advocate system that ingests docs, drafts technical content, prepares growth experiments, and produces product feedback. The application letter and proof-of-work are ready.",
    purpose: "application",
  });
